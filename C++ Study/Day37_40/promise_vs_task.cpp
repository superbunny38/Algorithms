...
//A std::promise allows you to manually set the value of a future at any specific line of code you choose,
// completely independent of when (or if) the function ends.
//monitors the server health
void ServiceD(promise<string> prom){
    bool is_fulfilled = false;
    bool serverIsUp = true; //simulated server health status
    while(true){
        //checking health
        
        if (serverIsUp && !is_fulfilled){
            //send a signal
            prom.set_value("Server Started");
            is_fulfilled = true;
        }
...
int main(){
    ...
    try{
        configProm.set_value(); //signal that configuration is done
    }
    catch(const exception& e){
